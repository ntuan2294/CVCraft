...
Usage:
    python -m cvcraft.generate_cv.rag.indexing.indexer           # ingest nếu DB rỗng
    python -m cvcraft.generate_cv.rag.indexing.indexer --reset   # xóa và ingest lại
    python -m cvcraft.generate_cv.rag.indexing.indexer --hf --max-records 1000
"""
import argparse
import sys
import time
...
def _index_sample_batch_with_retry(
    ...
    """index_sample_batch với retry khi gặp rate limit."""
    summary_count = 0
    bullet_count = 0
    delay = 10
    ...
    for i in range(0, len(samples), chunk_size):
        ...
        for attempt in range(5):
            try:
                counts = _index_sample_batch(store, chunk, source)
                summary_count += counts["summaries_indexed"]
                bullet_count += counts["bullets_indexed"]
                break
