...
function ScoreRing({ score }: { score: number }) {
  ...
  const label = score >= 80 ? 'Xuất sắc' : score >= 60 ? 'Khá tốt' : score >= 40 ? 'Trung bình' : 'Cần cải thiện'
  const r = 42
  ...
  return (
    <div className="flex flex-col items-center gap-3 text-center">
      ...
      <div>
        <p className="text-xs font-bold uppercase tracking-wide text-gray-500">Điểm phù hợp</p>
...
function EditCvContent() {
  ...
  useEffect(() => {
    const stored = sessionStorage.getItem('review_cv_jd')
    if (!stored) return
    sessionStorage.removeItem('review_cv_jd')
    try {
  ...
  const handleAnalyze = async () => {
    if (!cvFile) { setAnalyzeError('Vui lòng tải lên file CV'); return }
    if (!jdText.trim()) { setAnalyzeError('Vui lòng nhập hoặc tải JD trước khi phân tích'); return }
    ...
    try {
      ...
      const res = await fetch('/api/review-cv/analyze', { method: 'POST', body: form })
      const data = await res.json()
      if (!res.ok) throw new Error(data.detail ?? 'Phân tích thất bại')
      setAnalysisResult(data as AnalysisResult)
    } catch (err) {
      setAnalyzeError(err instanceof Error ? err.message : 'Phân tích thất bại')
    } finally {
      setAnalyzing(false)
    }
  }
        ...
        {!analysisResult && (
          <>
            <header className="text-center">
              <h1 className="text-2xl font-bold text-gray-900">Phân tích CV</h1>
              <p className="mt-1 text-sm text-gray-500">Tải CV và JD để nhận điểm phù hợp, nhận xét và gợi ý cải thiện.</p>
            </header>
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
              <section className="rounded-2xl border border-indigo-100 bg-white p-5 shadow-sm">
                ...
                <textarea
                  ...
                  rows={9}
                  placeholder="Dán mô tả công việc vào đây hoặc dùng nút Tải JD..."
              ...
              <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                <h2 className="mb-4 font-semibold text-gray-900">CV cần phân tích</h2>
                <label className="flex min-h-52 cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-gray-300 px-5 py-8 transition-colors hover:border-indigo-400 hover:bg-indigo-50/30">
                  ...
                  {cvFile ? (
                    <div className="text-center">
                      ...
                      <p className="mt-0.5 text-xs text-gray-400">Nhấn để đổi file</p>
                    </div>
                  ) : (
                    <div className="text-center">
                      <p className="text-sm font-medium text-gray-700">Chọn file CV</p>
                      <p className="mt-0.5 text-xs text-gray-400">PDF, DOCX, PNG, JPG</p>
                    </div>
                  )}
                  <input
                    ref={cvInputRef}
        ...
        {analysisResult && (
          <section className="mx-auto max-w-3xl rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-6 rounded-2xl border border-gray-200 bg-gray-50 p-5">
              <h2 className="text-xs font-bold uppercase tracking-wide text-gray-500">Nhận xét</h2>
            ...
            <div className="mt-4 rounded-2xl border border-indigo-100 bg-indigo-50/50 p-5">
              <h2 className="text-xs font-bold uppercase tracking-wide text-indigo-600">
                Gợi ý cải thiện ({analysisResult.suggestions.length})
              </h2>
