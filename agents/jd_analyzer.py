...
"""
from state import CVAgentState, JobRequirement
...
def jd_analyzer_node(state: CVAgentState) -> dict:
    """
    Node trong LangGraph.
    Nhận state, trả về dict các field cần update.
    """
    if not state.job_description:
        return {
            "messages": ["[JD Analyzer] Bỏ qua - không có JD"],
        }
    ...
    try:
        ...
        return {
            "messages": [f"[JD Analyzer] Lỗi: {str(e)}"],
        }
