...
from jd_search.core.models import JDDocument, JDRewrittenSections, JDSearchResult, JDSearchResponse
from jd_search.infrastructure.llm.factory import LLMFactory, call_with_structured_output
from jd_search.rag.vector_store import JDVectorStore
from jd_search.rag.indexing.jd_indexer import index_jd_samples, index_seed_samples
...
class JDSearchService:
    ...
    def _split_sentences(text: str) -> list[str]:
        """Split a long paragraph into individual sentences at `. [A-Z]` boundaries."""
        # Only split if text is long enough to likely contain multiple sentences
        if len(text) < 60:
            return [text]
        # Split at period/exclamation/question mark followed by whitespace + uppercase letter
        # Avoid splitting on common abbreviations like "e.g.", "i.e.", "vs.", "etc."
        parts = re.split(r'(?<=[.!?])\s+(?=[A-Z])', text)
        return [p.strip() for p in parts if p.strip()]

    @staticmethod
    def _text_to_bullets(text: str) -> list[str]:
        """Fallback: parse raw text into a clean list of bullet strings."""
        ...
        for raw_line in normalized.splitlines():
            ...
            if not line:
                continue
            # If line is a long paragraph (no bullet markers), split into sentences
            if len(line) > 120:
    ...
    def _clean_bullet_items(cls, items: list[str]) -> list[str]:
        ...
        for item in items:
            ...
            if not line:
                continue
            # LLM sometimes returns the whole section as one string — split into sentences
            if len(line) > 120:
