...
Source: tinixai/vietnamese-job-descriptions — chỉ lấy bản ghi year=2026.

Usage:
    python -m jd_search.rag.indexing.jd_indexer
    python -m jd_search.rag.indexing.jd_indexer --max-records 5000
    python -m jd_search.rag.indexing.jd_indexer --reset
    python -m jd_search.rag.indexing.jd_indexer --dry-run
"""
import argparse
import sys
from jd_search.rag.vector_store import JDVectorStore
from jd_search.rag.loaders.hf_jd_loader import load_hf_jd_samples
...
def _build_searchable_text(jd: dict, skip_keys: set = _SKIP_KEYS) -> str:
    """Ghép tất cả fields (trừ skip_keys) thành 1 string để embed."""
    parts = []
...
def index_seed_samples(reset: bool = False) -> dict:
    ...
    from jd_search.rag.seeds import JD_SEEDS
    ...
    ids, texts, metadatas = [], [], []
    for jd in JD_SEEDS:
        # Seed data dùng tên field khác (title, company, ...) — skip chỉ id
